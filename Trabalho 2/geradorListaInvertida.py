...
def read_escreva(arquivo):
    ...
    try:
        with open(arquivo, 'r') as file:
            for line in file:
                if 'ESCREVA=' in line:
                    escreva_data.append(line.split('ESCREVA=', 1)[1].strip())
        return escreva_data
    except FileNotFoundError:
        print("File 'gli.cfg' not found.")
...
# Gera lista invertida com stemmer 
def generate_stemmed_list(xml_files, output_file):
    inverted_list = {}
    stop_words = set(stopwords.words('english'))
    ...
    for xml_file in xml_files:
        ...
        for record in root.findall('.//RECORD'):
            ...
            # Realizando o stemming das palavras
...
generate_inverted_list(read_leia(cfg), read_escreva(cfg)[0])
generate_stemmed_list(read_leia(cfg), read_escreva(cfg)[1])